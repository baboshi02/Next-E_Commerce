...
import ItemInfoBtn from "../components/ItemInfoButton";
interface CartProps {
  productId: string;
  count: number
}
...
const CartProductElement = ({ productId, count }: CartProps) => {
  ...
  return (
    <>
      {
        !isError ? product &&
          <div className="flex flex-col flex-nowrap  items-center p-2   group hover:text-thirdColor hover:bg-primaryColor text-primaryColor transition duration-300 min-h-60 max-h-full   overflow-hidden">
            ...
            <h1>Count: {count}</h1>
            <ItemInfoBtn productId={productId} />
          </div> : <h1>Error</h1>
      }
